...
mkdir /etc/ger-frm/trojancert 1> /dev/null 2> /dev/null
...
openssl genrsa 2048 > /etc/ger-frm/trojancert/trojan.key
chmod 400 /etc/ger-frm/trojancert/trojan.key
openssl req -new -x509 -nodes -sha256 -days 365 -key /etc/ger-frm/trojancert/trojan.key -out /etc/ger-frm/trojancert/trojan.crt
clear
echo -e "\033[1;37mΔ Generando Configuracion"
sed -i '13i        "cert":"/etc/ger-frm/trojancert/trojan.crt",' /usr/local/etc/trojan/config.json
sed -i '14i        "key":"/etc/ger-frm/trojancert/trojan.key",' /usr/local/etc/trojan/config.json
