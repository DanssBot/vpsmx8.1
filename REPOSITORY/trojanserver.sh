...
mkdir /etc/ger-frm/trojan/trojancert 1> /dev/null 2> /dev/null
...
openssl genrsa 2048 > /etc/ger-frm/trojan/trojancert/trojan.key
chmod 400 /etc/ger-frm/trojan/trojancert/trojan.key
openssl req -new -x509 -nodes -sha256 -days 365 -key /etc/ger-frm/trojan/trojancert/trojan.key -out /etc/ger-frm/trojan/trojancert/trojan.crt
clear
echo -e "\033[1;37mΔ Generando Configuracion"
sed -i '13i        "cert":"/etc/ger-frm/trojan/trojancert/trojan.crt",' /usr/local/etc/trojan/config.json
sed -i '14i        "key":"/etc/ger-frm/trojan/trojancert/trojan.key",' /usr/local/etc/trojan/config.json
...
echo -ne "\033[1;32m [2] > " && msg -azu "$(fun_trans "INSTALAR Go Lang") "
msg -bar
